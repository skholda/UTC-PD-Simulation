"""
-5 V evaluation with -7 V L values (NO refit)
==============================================
L values are physically bias-independent → use the -7 V Option B fit as-is:
  L_total = 197.9 pH,  L_CPW2(38) = 56.3 pH,  L_CPW2(60) = 48.0 pH
Only Cj changes: 161 fF (C-V @ -5 V).
Overlay on -5 V measured S11 + freq response (5 mA).
"""
...
# -7 V locked L values (Option B), used AS-IS at -5 V
configs=[
    dict(lbl='Rp=200Ω', Rp=200.0, s1p='data_5V_5mA/S11_200ohm.s1p', fr='data_5V_5mA/200ohm.xlsx',
         col='#888888', mk='D', Lcpw=197.9e-12, Lcpw2=0.0,     Lrp=153.7e-12),
    dict(lbl='Rp=38Ω',  Rp=38.0,  s1p='data_5V_5mA/S11_38ohm.s1p',  fr='data_5V_5mA/38ohm.xlsx',
         col='#1B998B', mk='o', Lcpw=141.6e-12, Lcpw2=56.3e-12, Lrp=65.6e-12),
    dict(lbl='Rp=60Ω',  Rp=60.0,  s1p='data_5V_5mA/S11_60ohm.s1p',  fr='data_5V_5mA/60ohm.xlsx',
         col='#FF8C00', mk='s', Lcpw=149.9e-12, Lcpw2=48.0e-12, Lrp=71.8e-12),
    dict(lbl='Open',    Rp=np.inf,s1p='data_5V_5mA/S11_WO.s1p',     fr='data_5V_5mA/WO.xlsx',
         col='#E91E8C', mk='^', Lcpw=197.9e-12, Lcpw2=0.0,     Lrp=0.0),
]

f_plot=np.linspace(0.1e9,45e9,4000); w_plot=2*np.pi*f_plot
print('='*95)
print('-5 V evaluation with -7 V L values as-is (NO refit)  |  Cj = 161 fF')
print('  L_total=197.9 pH,  L_CPW2: 38Ω=56.3, 60Ω=48.0 pH,  FEM L_Rp')
print('='*95)
print(f'{"Device":>10} | {"RMS_S11 (noRefit)":>17} | {"RMS_S11 (-5V fit)":>17} | {"BW model":>8} | {"BW meas":>8} | {"RMS_H":>6}')
print('-'*95)
# reference: -5V refit values for comparison
rms5V_ref={'Rp=200Ω':0.06893,'Rp=38Ω':0.05777,'Rp=60Ω':0.05978,'Open':0.08183}
...
fig.suptitle('-5 V data with -7 V L values (bias-independent, NO refit)  |  Cj=161 fF (C-V @ -5 V)\n'
...
for ci,cfg in enumerate(configs):
    ...
    print(f'{cfg["lbl"]:>10} | {rms:>17.5f} | {rms5V_ref[cfg["lbl"]]:>17.5f} | '
    ...
    ax.plot(S.real,S.imag,'--',color='k',lw=1.5,zorder=5,label='Model (-7V L)')
    ...
    # Origin export (noRefit tag)
    t=cfg['lbl'].replace('Ω','ohm').replace('=','_')
    zm=(1+S11m)/(1-S11m); zf=(1+S)/(1-S)
    with open(f'origin_data_5V_5mA/s11_noRefit_{t}.txt','w') as fp:
    ...
    with open(f'origin_data_5V_5mA/impedance_noRefit_{t}.txt','w') as fp:
    ...
    with open(f'origin_data_5V_5mA/freqresp_noRefit_{t}.txt','w') as fp:
        fp.write('Freq_GHz\tNorm_dB_meas\tNorm_dB_model\n')
        for i in range(len(fm)):
            fp.write(f'{fm[i]/1e9:.6g}\t{pm[i]:.6g}\t{Hd_m[i]:.6g}\n')

fig.tight_layout()
fig.savefig('Eval_5V_locked_L.png',dpi=150,bbox_inches='tight')
print('\nSaved: Eval_5V_locked_L.png + origin_data_5V_5mA/*_noRefit_*.txt')
