"""
CASE 6 — single-L_CPW schematic with NO L_m:
   Iph || C_PD -[Rs]- node{ C_CPW || R_m } -[L_CPW]- port
Free parameters: L_CPW and C_CPW (per device).  L_m removed (set to 0).
Locked: C_PD=131 fF (C-V), Rs=8.92, R_m design.
Transit time: latest set (staircase tau_A = 1.989 ps, f_tr = 42.25 GHz).
Outputs: case6_smith_7V.png, case6_pout_7V.png, case6_tables.png
"""
...
def s11f(w,Rp,L,Lm,C):
    Z1=Rs+1/(1j*w*Cpd)
    Yrm=0.0 if np.isinf(Rp) else 1/(Rp+1j*w*Lm)
    Zin=1j*w*L+1/(1j*w*C+Yrm+1/Z1); return (Zin-50)/(Zin+50)
def H_ckt(w,Rp,L,Lm,C):
    ...
    sh(1j*w*Cpd); se(Rs+0j*w); sh(1j*w*C)
...
fit={}
print('CASE 6 fit (free: L_CPW, C_CPW; L_m removed)')
for lbl,Rp,s1pk,frk,Lm,col,xmax in DEV:
    a=_ns[s1pk]; ws=2*np.pi*a[:,0]; S=10**(a[:,1]/20)*np.exp(1j*np.deg2rad(a[:,2]))
    cost=lambda L,C: np.sqrt(np.mean(np.abs(s11f(ws,Rp,L*1e-12,Lm,C*1e-15)-S)**2))
    best=(1e9,None,None)
    for C in np.arange(5,220,2.0):
        Ls=np.arange(60,340,2.0); vals=np.array([cost(L,C) for L in Ls]); i=int(np.argmin(vals))
        if vals[i]<best[0]: best=(vals[i],Ls[i],C)
    ref=None
    for g in [(best[1],best[2]),(190,46.53),(160,90),(200,30)]:
        r=minimize(lambda p: cost(*p) if min(p)>0 else 1e3,g,method='Nelder-Mead',
                   options=dict(xatol=1e-4,fatol=1e-11,maxiter=9000))
        if ref is None or r.fun<ref.fun: ref=r
    fit[lbl]=(ref.x[0],ref.fun,ref.x[1])
    print(f'  {lbl:>8}: L_CPW={ref.x[0]:6.1f} pH  C_CPW={ref.x[1]:6.2f} fF   RMS={ref.fun:.5f}')
print(f'  total RMS = {sum(v[1] for v in fit.values()):.5f}   (CASE 5 with L_m = 0.24819)')
...
for i,(lbl,Rp,s1pk,frk,Lm,col,xmax) in enumerate(DEV):
    ...
    Sf=s11f(ws,Rp,fit[lbl][0]*1e-12,Lm,fit[lbl][2]*1e-15)
...
fig.tight_layout(); fig.savefig('case6_smith_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case6_smith_7V.png')

bw={}
fig,axes=plt.subplots(2,2,figsize=(13,10))
for i,(lbl,Rp,s1pk,frk,Lm,col,xmax) in enumerate(DEV):
    ax=axes[i//2,i%2]; L=fit[lbl][0]*1e-12; Cf=fit[lbl][2]*1e-15
    ...
    H0=H_ph(1e-3)*H_ckt(np.array([1e-3]),Rp,L,Lm,Cf)
    Hdp=20*np.log10(np.abs(H_ph(w_plot)*H_ckt(w_plot,Rp,L,Lm,Cf))/np.abs(H0))
    fw=np.linspace(0.05e9,80e9,16000)
    Hw=20*np.log10(np.abs(H_ph(2*np.pi*fw)*H_ckt(2*np.pi*fw,Rp,L,Lm,Cf))/np.abs(H0))
...
fig.tight_layout(); fig.savefig('case6_pout_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case6_pout_7V.png'); print('BW:',{k:round(v,1) for k,v in bw.items()})
...
gs=fig.add_gridspec(7,1,height_ratios=[1.30,0.98,0.12,0.42,0.34,0.42,0.10],
                    hspace=0.75,top=0.925,bottom=0.02,left=0.03,right=0.985)
fig.suptitle('Circuit-model parameters  (free parameters: $L_{CPW}$, $C_{CPW}$)',fontsize=15,fontweight='bold',y=0.975)
mk_table(fig.add_subplot(gs[0]),'Table I.  Fixed parameters',
    ...
    [['C_PD','131 fF (−7 V)','C–V measurement (locked)'],
     ...
     ['L_m','— (removed)','Shunt branch is R_m only']],
    [0.2,0.36,0.44])
mk_table(fig.add_subplot(gs[1]),'Table II.  Fitted parameters (S11 fit, −7 V)',
    ['Device','L_CPW (pH)','C_CPW (fF)','RMS of S11'],
    [[l.replace(' ohm',' Ω'),f'{fit[l][0]:.1f}',f'{fit[l][2]:.2f}',f'{fit[l][1]:.4f}'] for l,_,_,_,_,_,_ in DEV],
    [0.24,0.26,0.26,0.24])
mk_note(fig.add_subplot(gs[2]),
    'The shunt branch is a bare R_m (no L_m). For reference, the same circuit with the FEM L_m gives '
    f'a total S11 RMS of 0.2482 versus {sum(v[1] for v in fit.values()):.4f} here.')
...
fig.savefig('case6_tables.png',dpi=200,bbox_inches='tight',facecolor='white')
print('saved case6_tables.png')
