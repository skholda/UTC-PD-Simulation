"""
CASE 7 — single-L_CPW schematic:
   Iph || C_PD -[Rs]- node{ C_CPW || (R_m + L_m) } -[L_CPW]- port
Free parameters: L_CPW and L_m (per device), L_m constrained >= 0.
Locked: C_PD=131 fF (C-V), Rs=8.92, C_CPW=46.53 fF, R_m design.
The optimum drives L_m to 0 for every shunt device; unconstrained it goes
negative (-304.9 / -33.3 / -5.5 pH for 200 / 60 / 38 ohm).
Transit time: latest set (staircase tau_A = 1.989 ps, f_tr = 42.25 GHz).
Outputs: case7_smith_7V.png, case7_pout_7V.png, case7_tables.png
"""
...
def s11f(w,Rp,L,Lm):
    ...
    Zin=1j*w*L+1/(1j*w*Ccpw+Yrm+1/Z1); return (Zin-50)/(Zin+50)
def H_ckt(w,Rp,L,Lm):
    ...
    sh(1j*w*Cpd); se(Rs+0j*w); sh(1j*w*Ccpw)
    if not np.isinf(Rp): sh(1/(Rp+1j*w*Lm))
    se(1j*w*L); return R_L/(Cm*R_L+D)
...
DEV=[('WO', np.inf,'_s1p_WO','_freq_WO',0.0,'#E91E8C',30),
     ('200 ohm',200.0,'_s1p_200','_freq_200',0.0,'#888888',30),
     ('60 ohm', 60.0,'_s1p_55','_freq_55',0.0,'#FF8C00',35),
     ('38 ohm', 38.0,'_s1p_33','_freq_33',0.0,'#1B998B',35)]

# ── fit L_CPW per device ───────────────────────────────────────────
fit={}
print('CASE 7 fit (free: L_CPW, L_m>=0 -> L_m=0)')
LM_FEM={'WO':0.0,'200 ohm':153.7,'60 ohm':71.8,'38 ohm':65.6}
for lbl,Rp,s1pk,frk,Lm,col,xmax in DEV:
    a=_ns[s1pk]; ws=2*np.pi*a[:,0]; S=10**(a[:,1]/20)*np.exp(1j*np.deg2rad(a[:,2]))
    if np.isinf(Rp):
        c=lambda L: np.sqrt(np.mean(np.abs(s11f(ws,Rp,L*1e-12,0.0)-S)**2))
        r=minimize_scalar(c,bounds=(20,400),method='bounded'); fit[lbl]=(r.x,r.fun,0.0)
        print(f'  {lbl:>8}: L_CPW={r.x:6.1f} pH  L_m=  n/a    RMS={r.fun:.5f}'); continue
    def cost(p):
        if p[0]<=0 or p[1]<0: return 1e3
        return np.sqrt(np.mean(np.abs(s11f(ws,Rp,p[0]*1e-12,p[1]*1e-12)-S)**2))
    best=None
    for g in [(190,LM_FEM[lbl]),(160,60),(180,0),(200,150),(150,300)]:
        r=minimize(cost,g,method='Nelder-Mead',options=dict(xatol=1e-4,fatol=1e-11,maxiter=9000))
        if best is None or r.fun<best.fun: best=r
    fit[lbl]=(best.x[0],best.fun,max(best.x[1],0.0))
    print(f'  {lbl:>8}: L_CPW={best.x[0]:6.1f} pH  L_m={best.x[1]:6.1f} pH   RMS={best.fun:.5f}')
...
for i,(lbl,Rp,s1pk,frk,Lm,col,xmax) in enumerate(DEV):
    ...
    Sf=s11f(ws,Rp,fit[lbl][0]*1e-12,fit[lbl][2]*1e-12)
...
fig.tight_layout(); fig.savefig('case7_smith_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case7_smith_7V.png')

bw={}
fig,axes=plt.subplots(2,2,figsize=(13,10))
for i,(lbl,Rp,s1pk,frk,Lm,col,xmax) in enumerate(DEV):
    ax=axes[i//2,i%2]; L=fit[lbl][0]*1e-12; Lm=fit[lbl][2]*1e-12
...
fig.tight_layout(); fig.savefig('case7_pout_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case7_pout_7V.png'); print('BW:',{k:round(v,1) for k,v in bw.items()})
...
gs=fig.add_gridspec(7,1,height_ratios=[1.10,0.98,0.30,0.42,0.34,0.42,0.10],
                    hspace=0.75,top=0.925,bottom=0.02,left=0.03,right=0.985)
fig.suptitle('Circuit-model parameters  (free parameters: $L_{CPW}$, $L_m$)',fontsize=15,fontweight='bold',y=0.975)
...
mk_table(fig.add_subplot(gs[1]),'Table II.  Fitted parameters (S11 fit, −7 V)',
    ['Device','L_CPW (pH)','L_m (pH)','RMS of S11'],
    [['WO',f'{fit["WO"][0]:.1f}','—',f'{fit["WO"][1]:.4f}'],
     ['200 Ω',f'{fit["200 ohm"][0]:.1f}','0.0',f'{fit["200 ohm"][1]:.4f}'],
     ['60 Ω',f'{fit["60 ohm"][0]:.1f}','0.0',f'{fit["60 ohm"][1]:.4f}'],
     ['38 Ω',f'{fit["38 ohm"][0]:.1f}','0.0',f'{fit["38 ohm"][1]:.4f}']],
    [0.24,0.26,0.26,0.24])
mk_note(fig.add_subplot(gs[2]),
    'L_m is a free parameter constrained to be non-negative; the optimum sits at L_m = 0 for every\n'
    'shunt device. Unconstrained, it converges to -304.9 / -33.3 / -5.5 pH for 200 / 60 / 38 Ω\n'
    '(total S11 RMS 0.2585 versus 0.2674 here), i.e. an unphysical negative inductance.')
...
fig.savefig('case7_tables.png',dpi=200,bbox_inches='tight',facecolor='white')
print('saved case7_tables.png')
