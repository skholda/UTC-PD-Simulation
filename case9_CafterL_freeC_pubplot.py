"""
CASE 9 — L_CPW between the shunt tap and the pad capacitance:
...
fit={}
print('CASE 9 fit (free: L_CPW, C_CPW; C_CPW on the port side)')
for lbl,Rp,s1pk,frk,Lm,col,xmax in DEV:
    ...
    best=(1e9,None,None)
    for C in np.arange(2,220,2.0):
        Ls=np.arange(20,420,3.0); vals=np.array([cost(L,C) for L in Ls]); i=int(np.argmin(vals))
        if vals[i]<best[0]: best=(vals[i],Ls[i],C)
    ref=None
    for g in [(best[1],best[2]),(200,46.53),(160,20),(230,10)]:
...
print(f'  total RMS = {sum(v[1] for v in fit.values()):.5f}   (CASE 8 fixed C = 0.79219)')
...
fig.tight_layout(); fig.savefig('case9_smith_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case9_smith_7V.png')
...
fig.tight_layout(); fig.savefig('case9_pout_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case9_pout_7V.png'); print('BW:',{k:round(v,1) for k,v in bw.items()})
...
mk_note(fig.add_subplot(gs[2]),
    ...
    f'directly. Releasing it lowers the total S11 RMS to {sum(v[1] for v in fit.values()):.4f} from 0.7922 with C_CPW\n'
    'pinned at 46.53 fF.')
...
fig.savefig('case9_tables.png',dpi=200,bbox_inches='tight',facecolor='white')
print('saved case9_tables.png')
