"""
CASE 10 — L_CPW between the shunt tap and the pad capacitance:
   Iph || C_PD -[Rs]- node1{ L_m + R_m } -[L_CPW]- node2{ C_CPW } - port
Free parameters: L_CPW, C_CPW and C_PD (per device).
Locked: Rs=8.92, L_m=FEM, R_m design.
Note: C_PD is released here only as an experiment; the C-V value is 131 fF.
...
Rs=8.92; Ccpw=46.53e-15; R_L=50.0
...
fit={}
print('CASE 10 fit (free: L_CPW, C_CPW, C_PD)')
for lbl,Rp,s1pk,frk,Lm,col,xmax in DEV:
    a=_ns[s1pk]; ws=2*np.pi*a[:,0]; S=10**(a[:,1]/20)*np.exp(1j*np.deg2rad(a[:,2]))
    cost=lambda p: np.sqrt(np.mean(np.abs(s11f(ws,Rp,p[0]*1e-12,Lm,p[1]*1e-15,p[2]*1e-15)-S)**2)) if min(p)>0 else 1e3
    best=None
    for g in [(200,46.5,131),(230,40,120),(160,20,160),(180,80,100),(250,10,180),(140,5,200)]:
        r=minimize(cost,g,method='Nelder-Mead',options=dict(xatol=1e-4,fatol=1e-11,maxiter=15000))
        if best is None or r.fun<best.fun: best=r
    fit[lbl]=(best.x[0],best.fun,best.x[1],best.x[2])
    print(f'  {lbl:>8}: L_CPW={best.x[0]:6.1f} pH  C_CPW={best.x[1]:6.2f} fF  C_PD={best.x[2]:6.1f} fF   RMS={best.fun:.5f}')
print(f'  total RMS = {sum(v[1] for v in fit.values()):.5f}   (C_PD locked at 131 fF: 0.69929)')
...
for i,(lbl,Rp,s1pk,frk,Lm,col,xmax) in enumerate(DEV):
    ...
    Sf=s11f(ws,Rp,fit[lbl][0]*1e-12,Lm,fit[lbl][2]*1e-15,fit[lbl][3]*1e-15)
...
fig.tight_layout(); fig.savefig('case10_smith_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case10_smith_7V.png')

bw={}
fig,axes=plt.subplots(2,2,figsize=(13,10))
for i,(lbl,Rp,s1pk,frk,Lm,col,xmax) in enumerate(DEV):
    ax=axes[i//2,i%2]; L=fit[lbl][0]*1e-12; Cf=fit[lbl][2]*1e-15; Cd=fit[lbl][3]*1e-15
    ...
    H0=H_ph(1e-3)*H_ckt(np.array([1e-3]),Rp,L,Lm,Cf,Cd)
    Hdp=20*np.log10(np.abs(H_ph(w_plot)*H_ckt(w_plot,Rp,L,Lm,Cf,Cd))/np.abs(H0))
    fw=np.linspace(0.05e9,80e9,16000)
    Hw=20*np.log10(np.abs(H_ph(2*np.pi*fw)*H_ckt(2*np.pi*fw,Rp,L,Lm,Cf,Cd))/np.abs(H0))
...
fig.tight_layout(); fig.savefig('case10_pout_7V.png',dpi=170,bbox_inches='tight',facecolor='white')
print('saved case10_pout_7V.png'); print('BW:',{k:round(v,1) for k,v in bw.items()})
...
fig.suptitle('Circuit-model parameters  (free parameters: $L_{CPW}$, $C_{CPW}$, $C_{PD}$)',fontsize=15,fontweight='bold',y=0.975)
...
mk_table(fig.add_subplot(gs[1]),'Table II.  Fitted parameters (S11 fit, −7 V)',
    ['Device','L_CPW (pH)','C_CPW (fF)','C_PD (fF)','RMS of S11'],
    [[l.replace(' ohm',' Ω'),f'{fit[l][0]:.1f}',f'{fit[l][2]:.2f}',f'{fit[l][3]:.1f}',f'{fit[l][1]:.4f}'] for l,_,_,_,_,_,_ in DEV],
    [0.18,0.21,0.21,0.20,0.20])
mk_note(fig.add_subplot(gs[2]),
    f'Releasing C_PD lowers the total S11 RMS to {sum(v[1] for v in fit.values()):.4f} (0.6993 with C_PD locked at\n'
    '131 fF), but the fitted C_PD rises to 159-234 fF and grows monotonically as R_m falls, well\n'
    'away from the 131 fF C-V value; C_CPW simultaneously collapses toward 9-26 fF.')
...
fig.savefig('case10_tables.png',dpi=200,bbox_inches='tight',facecolor='white')
print('saved case10_tables.png')
