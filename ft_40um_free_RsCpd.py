"""40 um: free R_s and C_PD in the S11 fit (user-authorised). C_CPW locked.

Two variants:
  per-device : R_s, C_PD, L_CPW1, L_m, L_CPW2 all free for each device/bias
  joint      : one R_s shared by every 40 um device/bias, one C_PD per bias
               shared by the devices at that bias, per-device L's
...
def rms_of(d, Rs, Cpd, L1, Lm, L2):
    return np.sqrt(np.mean(np.abs(S11_g(d['w'], Rs, C_CPW_FIX, Cpd, d['Rm'],
                                        L1, Lm, L2) - d['Sm'])**2))

# ── variant A: per-device, 5 free ──────────────────────────────────────────
rowsA = []
for d in dev:
    keys = ['Rs', 'Cpd', 'L1', 'L2'] + ([] if d['open'] else ['Lm'])
    p0 = dict(Rs=8.92, Cpd=d['Cpd']*1e15, L1=80.0, L2=120.0, Lm=50.0)
    lo = dict(Rs=0.0, Cpd=50.0, L1=0.0, L2=0.0, Lm=0.0)
    hi = dict(Rs=60.0, Cpd=800.0, L1=600.0, L2=600.0, Lm=400.0)
    sc = dict(Rs=5.0, Cpd=30.0, L1=50.0, L2=50.0, Lm=50.0)
    def unpack(p):
        v = dict(zip(keys, p)); v.setdefault('Lm', 0.0)
        return v['Rs'], v['Cpd']*1e-15, v['L1']*1e-12, v['Lm']*1e-12, v['L2']*1e-12
    def resid(p):
        Rs, Cpd, L1, Lm, L2 = unpack(p)
        e = S11_g(d['w'], Rs, C_CPW_FIX, Cpd, d['Rm'], L1, Lm, L2) - d['Sm']
    ...
    Rs, Cpd, L1, Lm, L2 = unpack(r.x)
    rms = rms_of(d, Rs, Cpd, L1, Lm, L2)
    rms0 = rms_of(d, Rs_FIX, d['Cpd'],
                  *[base[(base.D == 40) & (base.V == d['V']) & (base.lab == d['lab'])]
                    .iloc[0][k]*1e-12 for k in ('L1', 'Lm', 'L2')])
    fRC = f3dB_of(fg, H_g(wg, Rs, C_CPW_FIX, Cpd, d['Rm'], L1, Lm, L2))/1e9
    rowsA.append(dict(V=d['V'], lab=d['lab'], Rm=d['Rm_meas'], Rs=Rs,
                      Cpd=Cpd*1e15, Cpd_CV=d['Cpd']*1e15,
                      L1=L1*1e12, Lm=Lm*1e12, L2=L2*1e12, rms=rms, rms_locked=rms0,
                      f_RC=fRC))
A = pd.DataFrame(rowsA)
pd.set_option('display.width', 220)
print('=== A. per-device free R_s, C_PD (40 um; C_CPW locked) ===')
print(A.to_string(index=False, float_format=lambda v: f'{v:.2f}'))

# ── variant B: joint, shared R_s and C_CPW ─────────────────────────────────
nd = len(dev)
BIAS = sorted({d['V'] for d in dev}, reverse=True)      # e.g. [-5, -7]
def unpackB(p):
    Rs = p[0]
    Cpd = {V: p[1+i]*1e-15 for i, V in enumerate(BIAS)}
    Ls = []; k = 1 + len(BIAS)
    for d in dev:
        if d['open']:
    ...
    return Rs, Cpd, Ls
def residB(p):
    Rs, Cpd, Ls = unpackB(p); out = []
    for d, (L1, Lm, L2) in zip(dev, Ls):
        e = S11_g(d['w'], Rs, C_CPW_FIX, Cpd[d['V']], d['Rm'], L1, Lm, L2) - d['Sm']
        out += [e.real, e.imag]
    return np.concatenate(out)
p0 = [8.92] + [C_PD_CV[(40, V)] for V in BIAS]
lo = [0.0] + [50.0]*len(BIAS); hi = [60.0] + [800.0]*len(BIAS)
sc = [5.0] + [30.0]*len(BIAS)
for d in dev:
    if d['open']:
...
RsB, CpdB, LsB = unpackB(r.x)
rowsB = []
for d, (L1, Lm, L2) in zip(dev, LsB):
    fRC = f3dB_of(fg, H_g(wg, RsB, C_CPW_FIX, CpdB[d['V']], d['Rm'], L1, Lm, L2))/1e9
    rowsB.append(dict(V=d['V'], lab=d['lab'], Rm=d['Rm_meas'],
                      Cpd=CpdB[d['V']]*1e15, Cpd_CV=d['Cpd']*1e15, L1=L1*1e12,
                      Lm=Lm*1e12, L2=L2*1e12,
                      rms=rms_of(d, RsB, CpdB[d['V']], L1, Lm, L2), f_RC=fRC))
B = pd.DataFrame(rowsB)
cpd_txt = ', '.join(f'C_PD({V} V) = {CpdB[V]*1e15:.1f} fF (C-V {C_PD_CV[(40, V)]:.1f})'
                    for V in BIAS)
print(f'\n=== B. joint fit, shared R_s = {RsB:.2f} ohm (locked {Rs_FIX});  {cpd_txt} ===')
...
axs[1].set_title(f'per-device: $R_s$ {A.Rs.min():.1f}–{A.Rs.max():.1f} $\\Omega$, '
                 f'$C_{{PD}}$ {A.Cpd.min():.0f}–{A.Cpd.max():.0f} fF', fontsize=10)
axs[2].set_title(f'joint: $R_s$ = {RsB:.2f} $\\Omega$, '
                 + ', '.join(f'$C_{{PD}}$({V} V) = {CpdB[V]*1e15:.0f} fF' for V in BIAS),
                 fontsize=10)
axs[0].set_title(f'locked: $R_s$ = {Rs_FIX} $\\Omega$, $C_{{PD}}$ from C–V '
                 f'(227.6 / 274.6 fF)', fontsize=10)
fig.tight_layout()
fig.savefig('ft_40um_free_RsCpd.png', dpi=300)
A.to_csv('ft_40um_free_RsCpd_perdevice.csv', index=False)
B.to_csv('ft_40um_free_RsCpd_joint.csv', index=False)
print('\nwrote ft_40um_free_RsCpd.png')
